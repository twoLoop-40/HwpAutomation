"""
실제 DB 원본 파일 테스트 - math-collector 로직 사용
"""
import sys
import codecs
...
# 실제 DB 파일 테스트
test_file = r"C:\db\mad-label\22개정 EBS 올림포스 기출문제집 공통수학2\22개정 EBS 올림포스 기출문제집 공통수학2_7.도형의 방정식2_3.hwp"
output_dir = Path("Tests/seperation/output_real_db")
output_dir.mkdir(parents=True, exist_ok=True)

print(f"=== 실제 DB 파일 테스트 ===\n")
...
with open_hwp(test_file) as hwp:
    ...
    saved_count = 0
    for i, (start, end) in enumerate(blocks[:min(3, len(blocks))], 1):
        print(f"블록 {i}: {start} → {end}")
...
print(f"\n완료! 총 {len(blocks)}개 블록 발견, {saved_count}개 저장 성공")
print(f"EndNote 앵커: {en_count}개")
